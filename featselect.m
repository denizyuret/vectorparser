...
function [best_feats, best_feats_err, cache] = featselect(x_tr,y_tr,x_te,y_te,degree,initfeats,cache)
...
if ~isKey(cache, fkey)
  ...
  tic();
  model_perceptron = k_perceptron_multi_train(x_tr_idx,y_tr,model_bak);
  pred_perceptron_last = model_predict(x_te_idx,model_perceptron,0);
  pred_perceptron_av = model_predict(x_te_idx,model_perceptron,1);
  telapsed = ...
      toc();
