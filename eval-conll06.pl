...
my ($usage) = <<EOT
  ...
  CoNLL-X evaluation script:
   ...
   [perl] eval.pl -g <gold standard> -s <system output>
...
our ($opt_g, $opt_s, $opt_o, $opt_h, $opt_v, $opt_q, $opt_p) ;
...
if (defined $opt_v)
{
    my $id = '$Id: eval.pl,v 1.7 2006/02/08 16:26:51 sabine Exp $';
    my @parts = split ' ',$id;
...
foreach $err (keys %err_counts)
{
  $thresh = (sort {$b <=> $a} values %{$err_counts{$err}})[$spec_err_loc_con-1] ;
